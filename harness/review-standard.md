...
> **当前状态：active。** 完整 review 循环已定义：Menglan 实现完成后直接触发 → Huahua 执行 → Menglan 修复（直接循环）→ Pandas 仅在 APPROVED 后介入 → Daniel HITL 合并。
...
| 角色 | Review 职责 | 限制 |
|---|---|---|
| **Pandas**（orchestrator） | 仅在 Huahua APPROVED 后接收 review_complete → tg_pr_ready Daniel；**不参与 review/fix 迭代循环** | **不读 PR diff，不发 review comments，不派发 code_review 或 fix_review** — 避免上下文污染 |
...
```
Menglan 完成 harness.sh implement
    └─▶ Menglan 直接写 code_review → for-huahua/pending/（直接循环，不经 Pandas）
            payload 含：req_id, pr_number, iteration
            └─▶ Huahua code_review handler（CodeX + GH LLM Issue Orchestrator）
                    └─▶ Findings 输出为 PR review comments
                            ├─▶ APPROVED → review_complete → for-pandas/pending/
                            │       └─▶ Pandas 接收后发 tg_pr_ready → Daniel [Merge] / [Hold]
...
收到 Huahua fix_review 消息（action=fix_review）或 Daniel 指令后，执行：
...
| 0.5 | 2026-03-26 | 修正设计漂移：Menglan 实现完成后直接派发 code_review（不经 Pandas）；Huahua NEEDS_CHANGES 时直接写 fix_review 给 Menglan（iter<2）或 tg_decision（iter≥2）；Pandas 仅在 APPROVED 后接收 review_complete；角色表、触发流程、Fix Review Findings 节同步修正 |
