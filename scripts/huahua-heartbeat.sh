...
INBOX="${SHARED_RESOURCES_ROOT:-${HOME}/shared-resources}/inbox/for-huahua"
...
main() {
  ...
  for msg_file in "${INBOX}"/*.md; do
    ...
    local type req_id pr_number summary
    ...
    case "$type" in
      tc_design)
        info "路由 tc_design → harness.sh tc-review ${pr_number:-$req_id}"
        bash "$REPO_ROOT/scripts/harness.sh" tc-review "${pr_number:-$req_id}"
        ;;
      review)
        info "路由 review → harness.sh fix-review ${pr_number:-$req_id}"
        bash "$REPO_ROOT/scripts/harness.sh" fix-review "${pr_number:-$req_id}"
        ;;
      *)
